...
class LoggerBrainNetFormer(object):
    ...
    def get(self, k=None, initialize=False):
        if self.k_fold is None:
            ...
            samples_prob = self.samples['prob']
            temp = []
            for index in range(len(samples_prob)):
                temp.append(samples_prob[index][:, 1])
            samples_prob = np.concatenate(temp)
            print(samples_prob)
            prob_1 = samples_prob
            ...
            if k is None:
                ...
                for k in range(self.k_fold):
                    ...
                    samples_prob = self.samples[k]['prob']
                    temp = []
                    for index in range(len(samples_prob)):
                        temp.append(samples_prob[index][:, 1])
                    samples_prob = np.concatenate(temp)
                    print(samples_prob)
                    prob_1[k] = samples_prob
            else:
        ...
        return dict(true=true, pred=pred,prob_1=prob_1)
    ...
    def evaluate(self, k=None, initialize=False, option='mean'):
        ...
        if not self.k_fold is None and k is None:
            ...
            roc_auc=0.5
            report=aggregate([classification_report(samples['true'][k], samples['pred'][k]) for k in range(self.k_fold)])
            print("report:  "+report)
            ...
            roc_auc = 0.5
            report = classification_report(samples['true'], samples['pred'])
            print("report:  " + report)
            # ! 这里有问题 炸炉了(已排查 是数据的问题)
            # roc_auc = metrics.roc_auc_score(samples['true'], self.samples[k]['prob'][:,1]) \
            #     if self.num_classes==2 \
            #     else metrics.roc_auc_score(samples['true'], samples['prob'], average='macro', multi_class='ovr')
            # roc_auc = metrics.roc_auc_score(samples['true'], samples_prob) \
            #     if self.num_classes==2 \
            #     else metrics.roc_auc_score(samples['true'], samples_prob, average='macro', multi_class='ovr')
        if initialize:
            self.initialize(k)
