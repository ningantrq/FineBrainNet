...
def step(model, criterion, sampling_points, sampling_endpoints, t, a, label,subtask_label
    ...
    # # @ subtask prediction
    logit_subtask=model(t.to(device),label)
    loss = criterion(logit_subtask, subtask_label.to(device))
...
def SubtaskTrain(argv):
    # make directories
    os.makedirs(os.path.join(argv.targetdir, 'Submodel'), exist_ok=True)
    os.makedirs(os.path.join(argv.targetdir, 'Subsummary'), exist_ok=True)
    ...
    if os.path.isfile(os.path.join(argv.targetdir, 'Subcheckpoint.pth')):
        print('resuming checkpoint experiment')
        checkpoint = torch.load(os.path.join(argv.targetdir, 'Subcheckpoint.pth'), map_location=device)
    else:
        checkpoint = {
            'fold': 0,
            'epoch': 0,
    ...
    for k in range(checkpoint['fold'], argv.k_fold):
        ...
        os.makedirs(os.path.join(argv.targetdir, 'Submodel', str(k)), exist_ok=True)

        # set dataloader
        dataset.set_fold(k, train=True)

        # define model
        model = SubtaskPredictor(
        ...
        summary_writer = SummaryWriter(os.path.join(argv.targetdir, 'Subsummary', str(k), 'train'), )
        summary_writer_val = SummaryWriter(os.path.join(argv.targetdir, 'Subsummary', str(k), 'val'), )
        # logger = util.logger.LoggerBrainNetFormer(argv.k_fold, dataset.num_classes)
        logger_subtask = util.sub_logger.LoggerBrainNetFormer(argv.k_fold, subtask_nums)
        ...
        for epoch in range(checkpoint['epoch'], argv.num_epochs):
            ...
            for i, x in enumerate(tqdm(dataloader, ncols=60, desc=f'k:{k} e:{epoch}')):  #
                ...
                # @ 向前传播+反向传播
                logit_subtask, loss_subtask = step(
                    model=model,
                    criterion=criterion,
                    ...
                    t=t,
                    a=a,
                    label=argv.subtask_labels[label.item()],
            ...
            torch.save({
                ...
                os.path.join(argv.targetdir, 'Subcheckpoint.pth'))

        # finalize fold
        torch.save(model.state_dict(), os.path.join(argv.targetdir, 'Submodel', str(k), 'model.pth'))
        checkpoint.update({'epoch': 0, 'model': None, 'optimizer': None, 'scheduler': None})

    summary_writer.close()
    summary_writer_val.close()
    os.remove(os.path.join(argv.targetdir, 'Subcheckpoint.pth'))


def SubtaskTest(argv):
    os.makedirs(os.path.join(argv.targetdir, 'Mainattention'), exist_ok=True)
    os.makedirs(os.path.join(argv.targetdir, 'Subattention'), exist_ok=True)
    ...
    for k in range(argv.k_fold):
        os.makedirs(os.path.join(argv.targetdir, 'Subattention', str(k)), exist_ok=True)
